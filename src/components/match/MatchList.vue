...
const textClass = computed(() => textToggle.value ? 'arrow-box opacity-100' : 'arrow-box opacity-0')
</script>
...
<template>
  <div id="match-list-container">
    ...
    <div v-if="matchRecommendToggle && !matchRecommendList" class="match-recommend-bg" style="display: flex; justify-content: center; align-items: center; color: white; font-weight: 700; font-size: 30px;">불러오는 중</div>
    <div v-if="matchRecommendToggle && matchRecommendList" class="match-recommend-bg" @click="matchRecommend"></div>
    <MatchRecommend v-if="matchRecommendToggle && matchRecommendList" :match-recommend-list="matchRecommendList"/>
    <div id="match-list-function">
      ...
      <button class="match-recommend-btn" @click="matchRecommend" @mouseover="mouseoverHandler" @mouseout="mouseoutHandler">
